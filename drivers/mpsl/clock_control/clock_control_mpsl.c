...
static enum clock_control_nrf_type manager_to_clock_type(struct onoff_manager *mgr) {
	__ASSERT_NO_MSG(PART_OF_ARRAY(onoff_managers, mgr));
	return ((enum clock_control_nrf_type)
		(POINTER_TO_UINT(mgr) -
		 POINTER_TO_UINT(onoff_managers)) / sizeof(*mgr));
}
...
struct onoff_manager *z_nrf_clock_control_get_onoff(clock_control_subsys_t sys)
{
	...
	return &onoff_managers[(unsigned) sys];
}
